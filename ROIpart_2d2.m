function [part1,seg1] = ROIpart_2d2(Image1,Segmentation1)

     [m,n,p]=size(Image1);   
    Segmentation = Segmentation1;
    Segmentation(Segmentation~=0)=1;
    [x,y,z] = ind2sub(size(Segmentation),find(Segmentation~=0));
    xrange = max(min(x),1):min(max(x),size(Segmentation,1));
    yrange = max(min(y),1):min(max(y),size(Segmentation,2));    
    zrange = max(min(z),1):min(max(z),size(Segmentation,3));
    ...
    num= max([length(xrange) length(yrange)]);
     ...
     zrange = max(zrange(1),1):min(zrange(end),size(Image1,3));
     
    
             
     
     part1 = Image1(xrange,yrange,zrange);
     seg1 = Segmentation1(xrange,yrange,zrange);
